...
interface UnifiedLoginFormProps {
  redirectUrl?: string; // original URL the user tried to visit (from ?redirect_url=...)
}
...
export default function UnifiedLoginForm({ redirectUrl }: UnifiedLoginFormProps) {
  const clerk = useClerk();
  const { isLoaded, isSignedIn, user } = useUser();

  const [mode, setMode] = useState<AuthMode>("signin");
  const [redirecting, setRedirecting] = useState(false);
  const [targetRole, setTargetRole] = useState<PortalRole>("student");
   ...
   * Core redirect logic.
   * Priority:
   * 1. If redirect_url was passed AND its domain matches the user's role portal → go there directly
   * 2. Otherwise → go to the role's default landing page
   */
  const performRedirect = (role: PortalRole) => {
    setRedirecting(true);
    setTargetRole(role);

    // Try honour the redirect_url if it belongs to the user's correct portal
    if (redirectUrl) {
      try {
        const redirectTarget = new URL(redirectUrl);
        const isLocal = redirectTarget.hostname === "localhost" || redirectTarget.hostname === "127.0.0.1";

        let rolePort: string | null = null;
        if (isLocal) {
          rolePort = role === "student" ? "3000" : role === "faculty" ? "3001" : "3002";
        }
        ...
        // Invalid redirect_url — fall through
      }
    }
    ...
    window.location.href = getPortalUrl(role, targetPath);
  };
  ...
  const handleSignInSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await clerk.client.signIn.create({ identifier, password });
      if (result.status === "complete") {
        await clerk.setActive({ session: result.createdSessionId });
        // useEffect above will fire and call performRedirect
      } else {
  ...
  const handleRequestResetCode = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!identifier) { setError("Please enter your registered email address."); return; }
  ...
  const handleResetPasswordSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!forgotCode || !newPassword) { setError("Please enter the verification code and new password."); return; }
    setLoading(true); setError("");
    try {
      if (!clerk.client?.signIn) return;
  ...
  const handleSignUpSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!signUpEmail || !signUpPassword || !firstName) { setError("Please fill in all required fields."); return; }
    setLoading(true); setError("");
    try {
      if (!clerk.client?.signUp) return;
      await clerk.client.signUp.create({ firstName, lastName, emailAddress: signUpEmail, password: signUpPassword });
  ...
  const handleVerifySignUpCode = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!signUpCode) { setError("Please enter the verification code."); return; }
    setLoading(true); setError("");
    try {
      if (!clerk.client?.signUp) return;
  ...
  if (redirecting || (isLoaded && isSignedIn)) {
    return (
      ...
      <div className="relative w-full max-w-5xl bg-white rounded-[32px] shadow-2xl shadow-indigo-100/70 border border-white p-6 sm:p-10 lg:p-12 flex flex-col md:flex-row gap-8 lg:gap-12 items-center overflow-hidden">
        ...
        <div className="w-full md:w-1/2 flex flex-col justify-center">
          ...
          {mode === "signin" && (
            <>
              <div className="mb-6">
                <h1 className="text-3xl sm:text-4xl font-black text-[#2C44B8] tracking-tight mb-2">Hello Again!</h1>
          ...
          {mode === "forgot" && (
            <div className="animate-fadeIn">
              <button type="button" onClick={() => { setMode("signin"); setError(""); setSuccessMessage(""); }} className="inline-flex items-center gap-1 text-xs font-semibold text-[#2C44B8] hover:underline mb-4 cursor-pointer">
                <ArrowLeft className="w-3.5 h-3.5" /> Back to Sign In
              </button>
              <h2 className="text-2xl font-black text-[#2C44B8] mb-2">Reset Password</h2>
              <p className="text-xs text-gray-400 mb-6">
                {forgotStep === 1 ? "Enter your email address and we'll send you a verification code." : "Enter the code sent to your email and set your new password."}
              </p>

              {error && <div className="bg-red-50 border border-red-200 text-red-600 text-xs rounded-xl p-3.5 mb-5 flex items-center gap-2 font-medium"><AlertCircle className="w-4 h-4 shrink-0" /><span>{error}</span></div>}
              {successMessage && <div className="bg-green-50 border border-green-200 text-green-700 text-xs rounded-xl p-3.5 mb-5 flex items-center gap-2 font-medium"><CheckCircle2 className="w-4 h-4 shrink-0 text-green-600" /><span>{successMessage}</span></div>}

              {forgotStep === 1 ? (
                <form onSubmit={handleRequestResetCode} className="space-y-4">
                  <input type="email" value={identifier} onChange={(e) => setIdentifier(e.target.value)} placeholder="Enter your registered email" className="w-full bg-[#F3F5FC] text-gray-800 placeholder:text-gray-400 rounded-2xl px-5 py-4 text-sm outline-none border border-transparent focus:border-[#2C44B8] focus:bg-white transition-all font-medium" required />
                  <button type="submit" disabled={loading} className="w-full bg-gradient-to-r from-[#32D4E3] to-[#1CCAE5] text-white font-bold tracking-widest text-sm uppercase rounded-2xl py-4 hover:shadow-lg hover:shadow-cyan-200/60 active:scale-[0.99] transition-all flex items-center justify-center gap-2 cursor-pointer">
                    {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : "SEND RESET CODE"}
                  </button>
                </form>
              ) : (
                <form onSubmit={handleResetPasswordSubmit} className="space-y-4">
                  <input type="text" value={forgotCode} onChange={(e) => setForgotCode(e.target.value)} placeholder="Enter 6-digit verification code" className="w-full bg-[#F3F5FC] text-gray-800 placeholder:text-gray-400 rounded-2xl px-5 py-4 text-sm outline-none border border-transparent focus:border-[#2C44B8] focus:bg-white transition-all font-medium" required />
                  <input type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="Enter new password" className="w-full bg-[#F3F5FC] text-gray-800 placeholder:text-gray-400 rounded-2xl px-5 py-4 text-sm outline-none border border-transparent focus:border-[#2C44B8] focus:bg-white transition-all font-medium" required />
                  <button type="submit" disabled={loading} className="w-full bg-gradient-to-r from-[#32D4E3] to-[#1CCAE5] text-white font-bold tracking-widest text-sm uppercase rounded-2xl py-4 hover:shadow-lg hover:shadow-cyan-200/60 active:scale-[0.99] transition-all flex items-center justify-center gap-2 cursor-pointer">
          ...
          {mode === "signup" && (
            <div className="animate-fadeIn">
              ...
              <p className="text-xs text-gray-400 mb-6">{signUpStep === 1 ? "Register to access NST PlacePrep" : "Enter verification code sent to your email"}</p>
              ...
              {signUpStep === 1 ? (
                <form onSubmit={handleSignUpSubmit} className="space-y-3.5">
                  <div className="grid grid-cols-2 gap-3">
                    <input type="text" value={firstName} onChange={(e) => setFirstName(e.target.value)} placeholder="First Name" className="w-full bg-[#F3F5FC] text-gray-800 placeholder:text-gray-400 rounded-2xl px-4 py-3.5 text-sm outline-none border border-transparent focus:border-[#2C44B8] focus:bg-white transition-all font-medium" required />
                    <input type="text" value={lastName} onChange={(e) => setLastName(e.target.value)} placeholder="Last Name" className="w-full bg-[#F3F5FC] text-gray-800 placeholder:text-gray-400 rounded-2xl px-4 py-3.5 text-sm outline-none border border-transparent focus:border-[#2C44B8] focus:bg-white transition-all font-medium" />
                  </div>
                  <input type="email" value={signUpEmail} onChange={(e) => setSignUpEmail(e.target.value)} placeholder="Email Address" className="w-full bg-[#F3F5FC] text-gray-800 placeholder:text-gray-400 rounded-2xl px-5 py-3.5 text-sm outline-none border border-transparent focus:border-[#2C44B8] focus:bg-white transition-all font-medium" required />
                  <input type="password" value={signUpPassword} onChange={(e) => setSignUpPassword(e.target.value)} placeholder="Create Password" className="w-full bg-[#F3F5FC] text-gray-800 placeholder:text-gray-400 rounded-2xl px-5 py-3.5 text-sm outline-none border border-transparent focus:border-[#2C44B8] focus:bg-white transition-all font-medium" required />
                  <button type="submit" disabled={loading} className="w-full bg-gradient-to-r from-[#32D4E3] to-[#1CCAE5] text-white font-bold tracking-widest text-sm uppercase rounded-2xl py-4 hover:shadow-lg hover:shadow-cyan-200/60 active:scale-[0.99] transition-all flex items-center justify-center gap-2 cursor-pointer mt-4">
                    {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : "CONTINUE"}
                  </button>
                </form>
              ) : (
                <form onSubmit={handleVerifySignUpCode} className="space-y-4">
                  <input type="text" value={signUpCode} onChange={(e) => setSignUpCode(e.target.value)} placeholder="Enter email verification code" className="w-full bg-[#F3F5FC] text-gray-800 placeholder:text-gray-400 rounded-2xl px-5 py-4 text-sm outline-none border border-transparent focus:border-[#2C44B8] focus:bg-white transition-all font-medium" required />
                  <button type="submit" disabled={loading} className="w-full bg-gradient-to-r from-[#32D4E3] to-[#1CCAE5] text-white font-bold tracking-widest text-sm uppercase rounded-2xl py-4 hover:shadow-lg hover:shadow-cyan-200/60 active:scale-[0.99] transition-all flex items-center justify-center gap-2 cursor-pointer">
